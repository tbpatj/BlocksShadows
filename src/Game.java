...
public class Game {
	...
	float gravSphereX = 0;
	float gravSphereY = 20;
	float gravSphereZ = 0;
	float gravPullDivide = 5;
	...
	public void input2()
	{
		...
		{
			float cX = cameraX + (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.sin(Math.toRadians(cameraDirY)) * -6);
			float cY = cameraY + (float)(Math.sin(Math.toRadians(cameraDirX)) * 6);
			float cZ = cameraZ + (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.cos(Math.toRadians(cameraDirY)) * -6);
			Position1 = new javax.vecmath.Vector3f(cX,cY,cZ);
		}
		if(Keyboard.isKeyDown(Keyboard.KEY_L))
		{
			if(placeSphere == false)
			{
				placeSphere = true;
				float cX = cameraX + (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.sin(Math.toRadians(cameraDirY)) * -6);
				float cY = cameraY + (float)(Math.sin(Math.toRadians(cameraDirX)) * 6);
				float cZ = cameraZ + (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.cos(Math.toRadians(cameraDirY)) * -6);
				createSphere(cX,cY,cZ,sliders.returnSliderValue(0),sliders.returnSliderValue(4),sliders.returnSliderValue(3),8);
			}
		}
		else
		{
		...
		{
			if(PlaceMovingPlatform == false)
			{
				PlaceMovingPlatform = true;
				float cX = cameraX + (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.sin(Math.toRadians(cameraDirY)) * -6);
				float cY = cameraY + (float)(Math.sin(Math.toRadians(cameraDirX)) * 6);
				float cZ = cameraZ + (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.cos(Math.toRadians(cameraDirY)) * -6);
				addPlatformAt(cX,cY,cZ,sliders.returnSliderValue(0),sliders.returnSliderValue(1),sliders.returnSliderValue(2),(float)(Math.random() * 1),(float)(Math.random() * 1),(float)(Math.random() * 1),sliders.returnSliderValue(4),sliders.returnSliderValue(5),sliders.returnSliderValue(6),sliders.returnSliderValue(7),sliders.returnSliderValue(8),sliders.returnSliderValue(9),sliders.returnSliderValue(10),true);
			}
		}
		else
		{
			PlaceMovingPlatform = false;
		}
		...
		{
			addBlockAt(cameraX,cameraY,cameraZ,1f,1f,1f,1f,1f,1f,0.1f,1f,0,0,0,0,0,0,false,objects,objectCollide,false);
		}
		float moveSpeed = 0.1f;
		 if(Keyboard.isKeyDown(Keyboard.KEY_W))
		 {
			 cameraX = cameraX - (float)(Math.sin(Math.toRadians(cameraDirY)) * moveSpeed);
			 cameraZ = cameraZ - (float)(Math.cos(Math.toRadians(cameraDirY)) * moveSpeed);
		 }
		 ...
		 {
			 cameraX = cameraX - (float)(Math.sin(Math.toRadians(cameraDirY - 90)) * moveSpeed);
			 cameraZ = cameraZ - (float)(Math.cos(Math.toRadians(cameraDirY - 90)) * moveSpeed);
		 }
		 ...
		 if(Keyboard.isKeyDown(Keyboard.KEY_DOWN))
		 {
			 ...
			 //cameraX = cameraX - (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.sin(Math.toRadians(cameraDirY)) * -5);
			 //cameraY = cameraY + (float)(Math.sin(Math.toRadians(cameraDirX))* 5);
			 //cameraZ = cameraZ - (float)(Math.cos(Math.toRadians(cameraDirX)) * Math.cos(Math.toRadians(cameraDirY)) * -5);
    ...
    public void renderObjects(boolean drawGround) {
        if (drawGround) {
            ...
            glBegin(GL_QUADS);
            glVertex3f(-100.0F, 0.25F, -100.0F);
            glVertex3f(-100.0F, 0.25F, 100.0F);
            glVertex3f(100.0F, 0.25F, 100.0F);
            glVertex3f(100.0F, 0.25F, -100.0F);
            glEnd();
        ...
        GL11.glPushMatrix();
        GL11.glTranslatef(gravSphereX, gravSphereY, gravSphereZ);
        Sphere drawSpheres = new Sphere();
        drawSpheres.setDrawStyle(GLU.GLU_LINE);
		GL11.glColor3f(0.5f, 0.5f, 0.5f);
		drawSpheres.draw(10, 10, 10);
		GL11.glPopMatrix();
		 ...
		 for(int i = 0; i < blocks.size(); i ++)
		 {
			 ...
			 float dis = (float)Math.sqrt((position.x - gravSphereX) * (position.x - gravSphereX) + (position.y - gravSphereY) * (position.y - gravSphereY) + (position.z - gravSphereZ) * (position.z - gravSphereZ));
			 if(dis > 10)
			 {
				 javax.vecmath.Vector3f towards = new javax.vecmath.Vector3f(position.x - gravSphereX,position.y - gravSphereY, position.z - gravSphereZ);
				 towards.x = towards.x / -gravPullDivide;
				 towards.y = towards.y / -gravPullDivide;
				 towards.z = towards.z / -gravPullDivide;
				 r.applyCentralForce(towards);
			 }
			 if(dis < 5)
			 {
				 javax.vecmath.Vector3f towards = new javax.vecmath.Vector3f(position.x - gravSphereX,position.y - gravSphereY, position.z - gravSphereZ);
				 towards.x = towards.x * 2;
				 towards.y = towards.y * 2;
				 towards.z = towards.z * 2;
				 r.applyCentralForce(towards);
			 }
			 */
	...
	public void setUpBlocks()
	{
		...
		for(int i = 0; i < 30; i ++)
		{
			...
			addBlockAt(0f,y,10,1f,1f,1f,1f,1f,1f,0.1f,1f,0,0,0,0,0,0,false,objects,objectCollide,false);
			y = y + 2;
		}
	}
	public void create()
	...
	public void setUpPhysics()
    {
    	...
    	MotionState groundMotionState = new DefaultMotionState(new Transform(new javax.vecmath.Matrix4f(new javax.vecmath.Quat4f(0,0,0,1),new javax.vecmath.Vector3f(0,0,0),1.0f)));
	...
	public void addCubeGround()
	{
    	...
    	vec.y = 2f;
    	CollisionShape cubeShape = new BoxShape(vec);
    	final Transform cubeTransform = new Transform(new javax.vecmath.Matrix4f(new javax.vecmath.Quat4f(0,0,(float)Math.toRadians(5),1),new javax.vecmath.Vector3f(0,-1.75f,-351),1.0f));
	...
	public void addTrees()
	{
		for(int i = 0; i < 10; i ++)
		{
			//addTreeAt((float)(Math.random() * 120 - 60), 0,(float)(Math.random() * 120 - 80));
		}
		setUpEnvironment();
	}
	public void setUpEnvironment()
	{
		...
		addBlockAt(10,20,0,1f,1f,2f,0.8f,0.8f,0.8f,1f,1f,0,0,0,0,0,0,false,objects,objectCollide,false);
		Block b3 = blocks.get(blocks.size() - 1);
		
		addBlockAt(10,20,0,1.5f,1f,2f,0.8f,0.8f,0.8f,1f,1f,0,0,0,0,0,0,false,objects,objectCollide,false);
		Block b4 = blocks.get(blocks.size() - 1);
		
		addBlockAt(10,20,0,1.5f,1f,0.1f,0.8f,0.8f,0.8f,1f,1f,0,0,0,0,0,0,false,objects,objectCollide,false);
		Block b5 = blocks.get(blocks.size() - 1);
		
		addBlockAt(10,20,0,1.5f,1f,0.1f,0.8f,0.8f,0.8f,1f,1f,0,0,0,0,0,0,false,objects,objectCollide,false);
		Block b6 = blocks.get(blocks.size() - 1);
		
		addBlockAt(10,24,0,1.5f,0.5f,2f,0.8f,0.8f,0.8f,1f,1f,0,0,0,0,0,0,false,objects,objectCollide,false);
		Block b7 = blocks.get(blocks.size() - 1);
		
		//createCyl(10,20,0,1f,1f,0.3f,1,1,1,1,1,10,10,nothing,objectCollide);
		//createCyl(10,20,0,1f,1f,0.3f,1,1,1,1,1,10,10,objects,objectCollide);
		//createCyl(10,20,0,1f,1f,0.3f,1,1,1,1,1,10,10,objects,objectCollide);
		//createCyl(10,20,0,1f,1f,0.3f,1,1,1,1,1,10,10,nothing,objectCollide);
		...
		con.setOrigins(0f, -2f, 0f, 0f, 0, 0f);
		createConstraint(b7.rigidBody,b.rigidBody,con,false,false);
		con.setOrigins(0f, 1f, 0f, 0f, 0, 0f);
		createConstraint(cube,b7.rigidBody,con,false,false);
		*/
   ...
   public void generateShadowMap() {
	...
	public void setUpOpenGL() {
	    ...
	    /*
	       ...
	       */
	    if (maxTextureSize > 2048 * 5) {
	        maxTextureSize = 2048 * 5;
   ...
   public void input() {
    ...
    private static void setUpBufferValues() {
        ...
        lightPosition.put(new float[]{200.0F, 100.0F, 20.0F, 1.0F});
        lightPosition.flip();
    }
}